...
public class WeeklyAppRecapFunction
{
    private readonly ILogger<WeeklyAppRecapFunction> _logger;
    ...
    private const string FeedUrl = "https://github.com/github/app/releases.atom";
    private const string StateFileName = "app-weekly-recap-last-date.txt";
    ...
    public WeeklyAppRecapFunction(
        ILogger<WeeklyAppRecapFunction> logger,
    ...
    [Function("WeeklyAppRecap")]
    public async Task Run([TimerTrigger("0 0 17,18 * * 6")] TimerInfo timerInfo)
    {
        _logger.LogInformation("WeeklyAppRecap function started at: {Time}", DateTime.UtcNow);
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Weekly App recap already posted for {Date}", todayKey);
                return;
            }
            ...
            {
                _logger.LogInformation("No App releases found for weekly window {Start} - {End}", weekStartUtc, weekEndUtc);
                return;
            }
            ...
            if (usePremiumMode)
            {
                var premiumPost = await _tweetFormatterService.FormatWeeklyAppRecapPremiumPostForXAsync(
                    weeklyEntries,
                    weekStartPacific,
                    weekEndPacific,
                    improvementCount,
                ...
                _logger.LogInformation("Formatted weekly App recap premium post ({Length} chars):\n{Post}",
            ...
            {
                var thread = await _tweetFormatterService.FormatWeeklyAppRecapThreadAsync(
                    weeklyEntries,
                    weekStartPacific,
                    weekEndPacific,
                    improvementCount,
                ...
                _logger.LogInformation("Formatted weekly App recap thread ({PostCount} posts, first {Length} chars):\n{Post}",
            ...
            if (success)
            {
                ...
                _logger.LogInformation("Successfully tweeted weekly App recap for {Date}", todayKey);
            }
            else
            {
                _logger.LogWarning("Failed to tweet weekly App recap for {Date}. Skipping.", todayKey);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error in WeeklyAppRecap function");
        }

        _logger.LogInformation("WeeklyAppRecap function completed at: {Time}", DateTime.UtcNow);
    }
